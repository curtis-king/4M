...
<head>
    ...
    <style>
        ...
        .sampling { display: grid; grid-template-columns: repeat(3, 1fr); border-top: 1px solid #cbd5e1; } .sampling > div { min-height: 45px; padding: 7px 8px; } .sampling > div + div { border-left: 1px solid #cbd5e1; }
...
<body>
    ...
    <main class="sheet">
        ...
        @if ($invoice->subject || $invoice->sample_nature || $invoice->company_site)
            <section class="box"><div class="box-title">Désignation de la mission</div><div class="sampling"><div><span class="label">Objet</span><span class="value">{{ $invoice->subject ?: '—' }}</span></div><div><span class="label">Nature des échantillons</span><span class="value">{{ $invoice->sample_nature ?: '—' }}</span></div><div><span class="label">Site de l'entreprise</span><span class="value">{{ $invoice->company_site ?: '—' }}</span></div></div></section>
        @endif
        @if ($invoice->is_statement)

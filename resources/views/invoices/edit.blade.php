<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-900 leading-tight">Modifier la facture {{ $invoice->number }}</h2>
    </x-slot>
        ...
        <div class="max-w-6xl mx-auto sm:px-6 lg:px-8">
            <form method="POST" action="{{ route('invoices.update', $invoice) }}" x-data="invoiceForm()" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div>
                            ...
                            <select name="agent_id" id="agent_id" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm">
                                ...
                                @foreach ($selectedClient?->agents ?? [] as $agent)
                                    <option value="{{ $agent->id }}" {{ $invoice->agent_id == $agent->id ? 'selected' : '' }}>
                                        {{ $agent->name }}{{ $agent->matricule ? ' ('.$agent->matricule.')' : '' }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        <div>
                            ...
                            <select name="recipient_type" id="recipient_type" required
                                ...
                                <option value="individual" {{ $invoice->recipient_type === 'individual' ? 'selected' : '' }}>Particulier</option>
                                <option value="business" {{ $invoice->recipient_type === 'business' ? 'selected' : '' }}>Entreprise</option>
                                <option value="government" {{ $invoice->recipient_type === 'government' ? 'selected' : '' }}>Gouvernement</option>
                                <option value="foreign" {{ $invoice->recipient_type === 'foreign' ? 'selected' : '' }}>Étranger</option>
                            </select>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <input type="date" name="due_date" id="due_date" value="{{ $invoice->due_date->format('Y-m-d') }}" required
                                class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm">
                        </div>
                        <div>
                            ...
                            <select name="currency" id="currency" required
                                class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm">
                                <option value="XAF" {{ $invoice->currency === 'XAF' ? 'selected' : '' }}>XAF (FCFA)</option>
                    ...
                    <div class="mt-6 border-t border-gray-200 pt-4">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div>
                                ...
                                <input type="text" name="subject" id="subject" value="{{ old('subject', $invoice->subject) }}"
                            ...
                            <div>
                                ...
                                <input type="text" name="sample_nature" id="sample_nature" value="{{ old('sample_nature', $invoice->sample_nature) }}"
                            ...
                            <div>
                                ...
                                <select name="insurance_contract_id" id="insurance_contract_id" x-model="selectedContractId"
                                    ...
                                    <template x-for="c in contracts" :key="c.id">
                                        <option :value="c.id" x-text="c.insurer.name + ' — ' + c.coverage_rate + '%'"></option>
                                    </template>
                                </select>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Couverture</label>
                                <div class="mt-2 text-sm text-gray-600" x-text="selectedContract ? selectedContract.coverage_rate + '%' : ''"></div>
                            </div>
                            <div>
                                <label for="pec_number" class="block text-sm font-medium text-gray-700">N° PEC</label>
                                <input type="text" name="pec_number" value="{{ $invoice->pec_number }}"
                                    class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm">
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="border border-gray-200 rounded-lg p-4 relative">
                                ...
                                <div class="grid grid-cols-12 gap-3">
                                    ...
                                    <div class="col-span-12 md:col-span-3">
                                        ...
                                        <input type="text" x-model="item.description" required class="w-full rounded-lg border-gray-300 shadow-sm text-sm">
                                    </div>
                                    ...
                                    <div class="col-span-3 md:col-span-1">
                                        ...
                                        <input type="number" x-model.number="item.quantity" min="0.01" step="0.01" @input="calcLine(index)" class="w-full rounded-lg border-gray-300 shadow-sm text-sm text-right">
                                    </div>
                                    <div class="col-span-3 md:col-span-2">
                                        <label class="block text-xs font-medium text-gray-500 mb-1">Prix unit.</label>
                                        <input type="number" x-model.number="item.unit_price" min="0" step="100" @input="calcLine(index)" class="w-full rounded-lg border-gray-300 shadow-sm text-sm text-right">
                                    </div>
                                    <div class="col-span-3 md:col-span-1">
                                        <label class="block text-xs font-medium text-gray-500 mb-1">Remise</label>
                                        <select x-model="item.discount_type" @change="calcLine(index)" class="w-full rounded-lg border-gray-300 shadow-sm text-sm">
                                            <option value="aucun">—</option>
                                    ...
                                    <div class="col-span-3 md:col-span-1">
                                        <label class="block text-xs font-medium text-gray-500 mb-1">Val.</label>
                                        <input type="number" x-model.number="item.discount_value" min="0" @input="calcLine(index)" class="w-full rounded-lg border-gray-300 shadow-sm text-sm text-right">
                                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div>
                            ...
                            <div class="flex gap-2">
                                ...
                                <input type="number" x-model.number="discount_value" min="0" :disabled="discount_type === 'aucun'" class="w-32 rounded-lg border-gray-300 shadow-sm text-sm text-right">
                            </div>
                        ...
                        <div>
                            ...
                            <textarea name="notes" rows="2" class="w-full rounded-lg border-gray-300 shadow-sm text-sm">{{ $invoice->notes }}</textarea>
                        </div>
                    </div>
                    <div class="mt-4 flex justify-end">
                        <div class="w-72 space-y-1 text-sm">
                            ...
                            <div class="flex justify-between" x-show="discountAmount > 0">
                                ...
                                <span class="font-medium" x-text="'- ' + formatNumber(discountAmount) + ' FCFA'"></span>
                            </div>
                            <div class="flex justify-between">
                            ...
                            <div class="flex justify-between border-t border-gray-300 pt-1">
                                ...
                                <span class="font-bold text-lg" x-text="formatNumber(total) + ' FCFA'"></span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex items-center justify-end gap-3">
                    <a href="{{ route('invoices.show', $invoice) }}" class="text-sm text-gray-600 hover:text-gray-900">Annuler</a>
    ...
    <script>
        function invoiceForm() {
            return {
                ...
                isWalkIn: {{ $invoice->client_id ? 'false' : 'true' }},
                ...
                companySiteMode: '{{ old('company_site', $invoice->company_site) }}',
                companySiteOther: '{{ old('company_site', $invoice->company_site) }}',
                ...
                get subtotal() { return this.items.reduce((s, i) => s + (parseFloat(i.net_amount) || 0), 0); },
                get discountAmount() {
                ...
                get taxAmount() { return (this.subtotal - this.discountAmount) * (this.tax_rate / 100); },
                get total() { return this.subtotal - this.discountAmount + this.taxAmount; },

                addItem() { this.items.push({ service_id: '', description: '', type: 'analyse', quantity: 1, unit_price: 0, discount_type: 'aucun', discount_value: 0, net_amount: 0 }); },
                ...
                calcLine(i) {
                    ...
                    let d = 0;
                    if (item.discount_type === 'pourcentage') d = lt * (item.discount_value / 100);
                    if (item.discount_type === 'montant') d = item.discount_value;

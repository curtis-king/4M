<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-900 leading-tight">Modifier la visite du {{ $visit->visit_date->format('d/m/Y') }}</h2>
    </x-slot>

    @php
        $editRows = $visit->examLines->map(fn ($l) => [
            'service_id' => (string) $l->service_id,
            'quantity' => (string) ($l->quantity ?: 1),
            'unit_price' => (string) ($l->unit_price ?? $l->service?->price ?? 0),
            'discount_type' => $l->discount_type,
            'discount_value' => (string) $l->discount_value,
            'resultat_valeur' => $l->resultat_valeur ?? '',
            'unite_mesure' => $l->unite_mesure ?? '',
            'valeur_limite' => $l->valeur_limite ?? '',
            'est_conforme' => $l->est_conforme === null ? '' : (string) ($l->est_conforme ? 1 : 0),
        ])->all();
        $editAgentIds = array_values(array_map('strval', old('agent_ids', $visit->agents->pluck('id')->all()) ?: []));
        $selectedClient = $clients->firstWhere('id', $visit->client_id);
    ...
    <div class="py-6">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-card rounded-2xl overflow-hidden">
                <form method="POST" action="{{ route('visits.update', $visit) }}" x-data="visitForm()" class="p-6 space-y-6">
                    @csrf @method('PUT')
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="beneficiary_name" id="beneficiary_name" value="{{ old('beneficiary_name', $visit->beneficiary_name) }}"
                        ...
                        <div>
                            ...
                            <input type="text" name="objet" id="objet" value="{{ old('objet', $visit->objet) }}" placeholder="Ex : Bilan annuel"
                                class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm">
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <select name="status" required
                                class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm">
                                @foreach (['planifiee' => 'Planifiée', 'realisee' => 'Réalisée', 'annulee' => 'Annulée', 'absent' => 'Absent'] as $val => $label)
                                    <option value="{{ $val }}" {{ old('status', $visit->status) === $val ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div x-show="isAlimentaire" x-transition class="bg-emerald-50/50 border border-emerald-100 rounded-xl p-4 space-y-4">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div>
                                ...
                                <input type="text" name="produit_alimentaire" id="produit_alimentaire" value="{{ old('produit_alimentaire', $visit->produit_alimentaire) }}"
                            ...
                            <div>
                                ...
                                <input type="text" name="numero_lot" id="numero_lot" value="{{ old('numero_lot', $visit->numero_lot) }}" placeholder="Ex : LOT-2026-045"
                                    class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 text-sm">
                            </div>
                            <div>
                                <label for="origine_produit" class="block text-sm font-medium text-gray-700">Origine du produit</label>
                                <input type="text" name="origine_produit" id="origine_produit" value="{{ old('origine_produit', $visit->origine_produit) }}" placeholder="Ex : Brasseur de Brazzaville"
                                    class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 text-sm">
                            </div>
                            <div>
                                <label for="date_prelevement" class="block text-sm font-medium text-gray-700">Date de prélèvement</label>
                                <input type="date" name="date_prelevement" id="date_prelevement" value="{{ old('date_prelevement', $visit->date_prelevement?->format('Y-m-d')) }}"
                                    class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 text-sm">
                            </div>
                            <div>
                                ...
                                <select name="type_controle" id="type_controle"
                                    ...
                                    <option value="routine" {{ old('type_controle', $visit->type_controle) === 'routine' ? 'selected' : '' }}>Routine</option>
                                    <option value="surveillance" {{ old('type_controle', $visit->type_controle) === 'surveillance' ? 'selected' : '' }}>Surveillance</option>
                                    <option value="plainte" {{ old('type_controle', $visit->type_controle) === 'plainte' ? 'selected' : '' }}>Plainte</option>
                                    <option value="certification" {{ old('type_controle', $visit->type_controle) === 'certification' ? 'selected' : '' }}>Certification</option>
                                </select>
                            </div>
                            <div>
                                ...
                                <select name="statut_resultat" id="statut_resultat"
                                    class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 text-sm">
                                    <option value="">— Sélectionner —</option>
                                    <option value="en_attente" {{ old('statut_resultat', $visit->statut_resultat) === 'en_attente' ? 'selected' : '' }}>En attente</option>
                                    <option value="conforme" {{ old('statut_resultat', $visit->statut_resultat) === 'conforme' ? 'selected' : '' }}>Conforme</option>
                                    <option value="non_conforme" {{ old('statut_resultat', $visit->statut_resultat) === 'non_conforme' ? 'selected' : '' }}>Non conforme</option>
                                </select>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div>
                            ...
                            <input type="number" step="0.01" min="0" name="patient_paid" id="patient_paid" value="{{ old('patient_paid', $visit->patient_paid) }}"
                                class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm">
                        ...
                        <div>
                            ...
                            <textarea name="notes" rows="2"
                                class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm">{{ old('notes', $visit->notes) }}</textarea>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center justify-end gap-3 pt-4">
                        <a href="{{ route('visits.show', $visit) }}" class="text-sm text-gray-600 hover:text-gray-900">Annuler</a>
    ...
    <script>
        function visitForm() {
            return {
                clientId: '{{ old('client_id', $visit->client_id) }}',
                clientSearch: @json($selectedClient?->name ?? ''),
                selectedContractId: '{{ old('insurance_contract_id', $visit->insurance_contract_id) }}',
                selectedAgentIds: @json($editAgentIds),
                clients: @json($clientOptions),
                contracts: @json($visit->insurance_contract_id
                    ? [$visit->insuranceContract->load('insurer')]
                    : []).map(c => ({ id: c.id, insurer: c.insurer ? c.insurer.name : 'Assureur', coverage_rate: c.coverage_rate, is_active: c.is_active })),
                agents: @json($agents),
                agentSearch: '',
                services: @json($serviceOptions),
                prestationSearch: '',
                catFilter: 'tous',
                rows: @json(old('exam_items', $editRows)),
                globalDiscount: @json((float) old('discount_value', $visit->discount_value)),
                isAlimentaire: @json(old('produit_alimentaire') || old('numero_lot') || old('statut_resultat') || $visit->is_controle_alimentaire),
                coverageRate: 0,
                ...
                loadClientData() {
                    ...
                    fetch(`/invoices/api/client/${this.clientId}`).then(r => r.json()).then(d => {
                        ...
                        const activeContracts = (d.client.insurance_contracts || []).filter(c => c.is_active);
                        this.contracts = activeContracts.map(c => ({
                            id: c.id,
                            insurer: c.insurer ? c.insurer.name : 'Assureur',
                            coverage_rate: c.coverage_rate,
                            is_active: c.is_active,
                        }));
                        if (this.contracts.length && !this.contracts.find(c => String(c.id) === String(this.selectedContractId))) {
